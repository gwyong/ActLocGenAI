...
def testing_coin_v5(path_gt_second_action_dict, path_pred_second_action_dict, output_folder_path="./output", save=True, save_fig=False):
    ...
    for video_path in tqdm(pred_second_action_dicts.keys()):
        ...
        for i in range(len(sorted_gt_seconds)):
            ...
            if isinstance(pred_action_top_k_list, str):
                pred_action_top_k_list = [pred_action_top_k_list]
            if len(pred_action_top_k_list) == 0:
                pred_action_top_k_list = ["none"]
...
def testing_coin_v6(path_gt_second_action_dict, path_pred_second_action_dict, output_folder_path="./output", K=5, save=True, save_fig=False):
    # TODO: Someday, we will update this function.
    # NOTE: For Gunwoo's Annotation & Top-K & Action-Object pairs

    df_dataset_actions = pd.read_csv("./COIN_videos/filtered_COIN_videos/task_step.csv")
    dataset_actions = df_dataset_actions["unified_3"].unique().tolist()
    dataset_action_conversion = {}
    for i in range(len(df_dataset_actions)):
        key = "---".join([df_dataset_actions.iloc[i]["Task"], df_dataset_actions.iloc[i]["Step"]])
        value = df_dataset_actions.iloc[i]["unified_3"]
        dataset_action_conversion[key] = value
    ...
    for video_path in tqdm(pred_second_action_dicts.keys()):
        ...
        for i in range(len(sorted_gt_seconds)):
            ...
            if len(pred_action_top_k_list) == 0:
                pred_action_top_k_list = ["none"]
            # trim the list to the top K actions
            if len(pred_action_top_k_list) > K:
                pred_action_top_k_list = pred_action_top_k_list[:K]
            ...
            gt_action = gt_second_action_dict[gt_second] # step
            if gt_action != "none":
                video_name = os.path.basename(video_path).replace(".mp4", "")
                key = video_name.split("_")[0] + "---" + gt_action
                gt_action = dataset_action_conversion[key]
...
def visualize_results(path_gt_second_action_dict, path_pred_second_action_dict, output_folder_path="./output", save=True):
    ...
    dataset_actions = pd.read_csv("./COIN_videos/filtered_COIN_videos/task_step.csv")["unified_3"].unique().tolist()
...
if __name__ == "__main__":
    ...
    path_pred_second_action_dict = "./output/predictions_gpt-4o_4.json"
    ...
    testing_coin_v6(path_gt_second_action_dict, path_pred_second_action_dict, output_folder_path="./output", K=5, save=True, save_fig=True)
